...
async function stopInvite(inputID, username, nameSpaceID, timeOut, buttonID)
{
	if (timeOut)
	{
		...
		.then(async data => {
			if (data.status == true)
			{
				...
				if (data.is_active == 1)
				{
					...
					stopInvite(inputID, clientUsername, nameSpaceID, 0, buttonID);
				}
				if (data.is_active == 2)
				{
					const div = document.getElementById(nameSpaceID);
...
async function invitePlayer(inputID, nameSpaceID, buttonID, username) {
	...
	intervalHandler["setTimeout"] = setTimeout(() => {
		stopInvite(inputID, clientUsername, nameSpaceID, 1, buttonID);
		clearInterval(intervalHandler[inputID]);
	}, 15000);
}
